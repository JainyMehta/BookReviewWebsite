...
app.post("/add", async (req, res) => {
  ...
  try {
    ...
    const result = await db.query(
      "INSERT INTO book (title, author, dateOfCompletion, feedback, ratings, olid) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *;",
      [
        data.title,
        data.author,
...
app.post("/edit", async (req, res) => {
  ...
  try {
    const result = await db.query(
      "UPDATE book SET title = $1, author = $2, feedback = $3, ratings =$4, olid = $5 WHERE id = $6;",
      [
        data.title,
        data.author,
        data.feedback,
